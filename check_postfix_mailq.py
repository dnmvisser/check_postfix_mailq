...
def validate_email(input):
    ...
    if not pattern.match(input):
        print(
            'UNKNOWN: Failed to validate sender filter, expecting "%s"'
            % default_re_email()
        )
        exit(3)
    if input[0] == "@":
...
def check_mailq(
    ...
    re_sender = compile(
        r"^[0-9A-F]{10}([!\*]?)\s+([0-9]+)\s+.*\s+(%s)$" % sender_filter, IGNORECASE
    )
    ...
    perfdata = [
        ...
        "recipients=%i;%i;%i;;"
        % (sum_mailq_recipients, recipients_warning, recipients_critical),
    ]
    if perfdata_details:
    ...
    if recipients_critical > 0 and sum_mailq_recipients >= recipients_critical:
        return 2, "CRITICAL: recipient count in mailq for filtered sender >%i | %s" % (
            recipients_critical,
            perfdata,
        )

    if recipients_warning > 0 and sum_mailq_recipients >= recipients_warning:
        return 1, "WARNING: recipient count in mailq for filtered sender >%i | %s" % (
            recipients_warning,
            perfdata,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=False,
        help=(
            "Generate critical if sum of all recipients exceeds this "
    ...
    if (
        args.recipients_warning > 0
        and args.recipients_warning >= args.recipients_critical
    ):
    ...
    ret = check_mailq(
        mailq,
        sender_filter,
        args.perfdata_details,
        args.count_warning,
        args.count_critical,
        args.size_warning,
        args.size_critical,
        args.recipients_warning,
        args.recipients_critical,
    )
    print(ret[1])
    exit(ret[0])
